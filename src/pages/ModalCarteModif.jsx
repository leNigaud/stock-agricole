...
const ModalCarteModif = ({hideEditModal, produit, conteneurs, contenance, setProduitConfirm, produitConfirm, setContenance, })=> {
    //css
        ...
        if (clicked) {
            ...
            const updatedContenance = contenance.map(item => {
                if (item.nom === nomToChange) {
                    let newCapacite = item.capacite

                    if(inputValue > before) newCapacite = parseInt(item.capacite) + parseInt(before) - parseInt(inputValue)
                    else if(inputValue < before) newCapacite = parseInt(item.capacite) + parseInt(before) - parseInt(inputValue) 
            ...
            if (produitConfirm.length != 0) {
                 newPr = produitConfirm.map((pr) => {
                    if (pr.libelle == updatedProduit.libelle) {
                        found = true
                        return { ...pr, uniteEntree: parseInt(inputValue)};
                    } else {
                        return pr; 
                    }
                });
...
export default ModalCarteModif;