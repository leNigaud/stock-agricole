...
function Modaleditdestination( {triggerUpdate, destination,handleHideEdit} ) {
    ...
    const hideModalSuccessModif = () => {
        setSuccess(false)
        handleHideEdit(destination.idD)
        triggerUpdate()
    }
        ...
        <div className='d-flex justify-content-center align-items-center' style={stylecontainer}>
            ...
            <form style={stylediv} onSubmit={handleOnSubmit} className="d-flex flex-column justify-content-center bg-white">
                <div  className='form-group mb-4'>
                    <label className="d-block mb-2" htmlFor="LieuD">Destination</label>
                    <input name="LieuD" id="LieuD" value={data.LieuD} onChange={handleChange} className='form-control' required/>
                </div>
                <div className='d-flex flex-row justify-content-center align-items-center'>
                    <div><button type="submit" className="btn btn-success btn-md" style={{marginRight:'20px'}} >Modifier</button></div>
                    <div><button type='button' className="btn btn-secondary btn-md" onClick={()=>handleHideEdit(destination.idD)}>Annuler</button></div>
                </div>
            </form>
        </div>
    )
